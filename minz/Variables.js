...
class Variables {
    ...
    getDimSeriePipeline(varCode, startTime, endTime, filter, groupDimension, extraDimData) {
        let v = this.variables[varCode];
        if (!v) throw("Variable '" + varCode + "' not found");
        ...
        let projection = {$project:{value:1, n:1, min:1, max:1, sum2:1, "dim.code":1, "dim.name":1, "dim.order":1}};
        if (extraDimData) projection.$project["dim." + extraDimData] = 1;
        pipe.push(projection);
        return pipe;
    }
    ...
    async getDimTree(varCode, startTime, endTime, filter, topDim, bottomDim) {
        if (topDim == bottomDim) {
            return await this.getDimSerie(varCode, startTime, endTime, filter, topDim);
        }
        let path = "" + bottomDim;
        ...
        rows = rows.map(r => {
            let r2 = r.dim;
            r2.min = r.min; r2.max = r.max;
            r2.n = r.n; r2.value = r.value; 
            r2.sum2 = r.sum2;
            return r2;
        })

        for (let i=0; i<topRest.length; i++) {
            let children = {};
            for (let row of rows) {
                let codDimPadre = row[topRest[i]];
                let childrenRows = children[codDimPadre];
                if (!childrenRows) {
                    childrenRows = [];
                    children[codDimPadre] = childrenRows;
                }
                childrenRows.push(row);
            }
            path += "." + topRest[i];
            let parentDimension = this.getFinalDimensionFromPath(varCode, path)
            let parentDimRows = await dimensions.getAllRows(parentDimension.code);
            let newRows = [];
            for (let parentRow of parentDimRows) {
                let child = children[parentRow.code];
                if (child) {
