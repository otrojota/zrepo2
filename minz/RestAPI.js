...
class RestAPI {
    ...
    register(app) {
        ...
        app.get("/data/:code/dim-tree", async (req, res) => {
            try {
                await security.checkPrivilege(this.getAuth(req), "minz-read");
                let startTime = req.query.startTime;
                let endTime = req.query.endTime;
                ...
                let filter = req.query.filter || "{}";
                filter = JSON.parse(filter);
                let topDim = req.query.topDim;
                if (!topDim) throw "Must provide topDim";
                let bottomDim = req.query.bottomDim;
                if (!bottomDim) throw "Must provide bottomDim";
                res.setHeader('Content-Type', 'application/json');
                let rows = await variables.getDimTree(req.params.code, startTime, endTime, filter, topDim, bottomDim);
                res.send(JSON.stringify(rows));
            } catch(error) {
                if (typeof error == "string") {
                    res.status(400).send(error.toString())
