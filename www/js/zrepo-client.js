...
class ZRepoClient {
    ...
    async readMetadata(force = false) {
        if (force) {
            this.dimensiones = null; this.variables = null; this.dataSets = null;
        }
    ...
    extraeAcumulador(resultado, acumulador) {
        ...
        if (Array.isArray(resultado)) {
            for (let row of resultado) {
                this.extraeAcumulador(row, acumulador);
            }
        } else {
            let r;
            switch(acumulador) {
                case "sum":
                case "value": r = resultado.value !== undefined?resultado.value:null; break;
                case "min": r = resultado.min !== undefined?resultado.min:null; break;
                case "max": r = resultado.max !== undefined?resultado.max:null; break;
                case "n"  : r = resultado.n !== undefined?resultado.n:0; break;
                case "avg": 
                    if (resultado.n > 0) {
            ...
            if (resultado.children && Array.isArray(resultado.children)) {
                this.extraeAcumulador(resultado.children, acumulador);
            }
            return r;
        }
    }
    query(query, startTime, endTime) {    
        ...
        */
        try {
            let filtro = query.filtro || {}, resultado;
            switch (query.tipoQuery) {
                ...
                case "dim-serie": {
                    ...
                    break;                
                case "dim-tree": {
                        let {promise, controller} = this.queryDimTree(query.variable.code, startTime, endTime, filtro, query.topDim, query.bottomDim);
                        let buildPromise = new Promise((resolve, reject) => {
                            promise.then(res => {
                                this.extraeAcumulador(res, query.acumulador);
                                resolve(res);
                            }).catch(err => reject(err))
                        })
                        resultado = {promise:buildPromise, controller}
    ...
    queryDimTree(codigoVariable, startTime, endTime, filter, topDim, bottomDim) {
        try {
            let url = this.url + "/data/" + codigoVariable + "/dim-tree?token=" + this.token;
            url += "&startTime=" + startTime + "&endTime=" + endTime;
            url += "&filter=" + encodeURIComponent(JSON.stringify(filter));
            url += "&topDim=" + topDim;
            url += "&bottomDim=" + bottomDim;
            let controller = new AbortController();
            //console.log("zrepo query url", url)
            return {promise: this._getJSON(url, controller.signal), controller:controller}
        } catch(error) {
