...
class MinZQuery {
    ...
    async describeFiltro(filtro) {        
        try {
            if (filtro.ruta.length == 0) {
                // variables es una dimensión y se filtra en el primer nivel
                let dimCode = this.variable.code;
                let row = await this.zRepoClient.getValorDimension(dimCode, filtro.valor);
                if (!row) return {etiqueta:this.variable.name, etiquetaValor:"[" + filtro.valor + "] Fila No Encontrada"}
                return {etiqueta:this.variable.name + " igual a '" + row.name + "'", etiquetaValor:row.name}
            }
    ...
    async construyeDescripcionFiltros() {
        try {
            ...
            let ret = [];
            for (let i=0; i<this.fixedFilters.length; i++) {
                let etiquetas = await this.describeFiltro(this.fixedFilters[i]);
                ret.push({
                    etiqueta:etiquetas.etiqueta,
                    etiquetaValor:etiquetas.etiquetaValor,
                    fijo:true,
                    ruta:this.fixedFilters[i].ruta,
                    valor:this.fixedFilters[i].valor
                });
            }
    ...
    async construyeArbolFiltrosDesde(nodos, dimOVar, path0, x0, y0, subArbolHabilitado, max) {
        ...
        for (let i=0; i<dimOVar.classifiers.length; i++) {
            ...
            if (filtro) {
                ...
                nodo.editable = filtro.fixed?false:true;
            } else if (subArbolHabilitado) {
                // Si es parte de la ruta del filtro fijo, se deshabilita
                /*
                if (this.allFiltros.find(f => f.fixed && f.filtro.ruta.startsWith(path))) {
                    nodo.editable = false;
                }
                */
            }
    ...
    async getArbolFiltros() {
        ...
        let hayFiltroRaiz = this.filters.findIndex(f => f.ruta.length == 0) >= 0;
        await this.construyeArbolFiltrosDesde(nodos, this.variable, "", 1, 0, hayFiltroRaiz?false:true, max);
        return {max:max, nodos:nodos};
    }
    agregaFiltro(ruta, valor) {
        // Si es raíz (dimension) se eliminan todos los filtros existentes
        if (ruta.length == 0) {
            this.filters.forEach(f => this.eliminaFiltro(f));
        }
    ...
    query(args) {
        ...
        let fixedFilters = this.fixedFilters?JSON.parse(JSON.stringify(this.fixedFilters)):null;
        if (args.objectCode && fixedFilters) {
            if (fixedFilters.valor == "${codigo-objeto}") fixedFilters.valor = args.objectCode;
        }
        // Construir filtro comun a todas las queries
        let filtro = {};
        this.fixedFilters.forEach(f => this.zRepoClient.construyeFiltro(filtro, f.ruta, f.valor));
        this.filters.forEach(f => this.zRepoClient.construyeFiltro(filtro, f.ruta, f.valor));
        //
        let q;
        if (args.startTime) {
        ...
        } else if (args.format == "dim-tree") {
            q = {
                tipoQuery:"dim-tree", 
                filtro:filtro, 
                variable:this.variable, 
                topDim:args.topDim,
                bottomDim:args.bottomDim,
                acumulador:this.accum,
                temporality:this.temporality
            }
