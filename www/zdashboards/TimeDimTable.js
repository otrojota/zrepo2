class TimeDimTable extends ZDashboardElement {
    get code() {return "time-dim-table"}
    ...
    async refresh(start, end, operation = "refresh") {
        try {
            ...
            if (!this.q || !this.options.rutaH) return;            
            if (operation == "refresh") {
                this.q.groupingDimension = this.options.rutaH;
            ...
            data = data.map(d => ({
                hName:d.dim.name, hCode:d.dim.code, hOrder:d.dim.order,
                vName:this.formatLocalTime(d.localTime, this.options.timeFormat), vCode: this.formatLocalTime(d.localTime, this.options.timeFormat),
                valor:d.resultado
            }));
            ...
            */
            ...
            for (let iRow=0; iRow<filas.length; iRow++) {
                ...
                html += `
                    <tr class="dim-dim-table-row">
                        <th class="dim-dim-table-row-hdr" style="white-space: nowrap;">${fila.name}</th>
                `;
                for (let iCol=0; iCol<columnas.length; iCol++) {
                    let columna = columnas[iCol];
