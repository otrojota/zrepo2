class DimSerie extends ZDashboardElement {
    ...
    async refresh(start, end, operation = "refresh") {
        try {
            ...
            // Ajustar alto mínimo de acuerdo a la cantidad de datos
            let chartContainer = this.find("#chartContainer-" + this.zId);
            if (this.options.serieType == "bars") {
                let minHeight = nRows * 45;
                chartContainer.style.setProperty("min-height", minHeight + "px");
            } else {
