...
class Portal extends ZModule {
    ...
    async _getUserSession(token) {
        try {
            let col = await mongoRead.collection("user_sesion");
    ...
    async login(email, pwd) {
        try {
            let u;
            let col = await mongoWrite.collection("users");
            ...
            col = await mongoWrite.collection("user_sesion");
    ...
    async autoLogin(token) {
        try {
            let col = await mongoWrite.collection("user_sesion");
            let s = await col.findOne({_id:token});
            if (!s) throw "Sesión Inválida (Token expirado)";
            col = await mongoWrite.collection("users");
            let u = await col.findOne({_id:s.email}, {projection:{_id:1, name:1, admin:1}});
            if (!u)  throw "Sesión Inválida (Usuario inválido: " + s.email + ")";
            s.user = u;
            s.token = token;
            delete s.email;
            return s;
    ...
    async getUsers() {
        try {
            let col = await mongoRead.collection("users");
    ...
    /* Procesos */
    async getProcessMetadata(pluginCode, processCode) {
        try {
            let md = await pluginAPI.getProcessMetadata(pluginCode, processCode);
            return md;
        } catch (error) {
            throw error;
        }
