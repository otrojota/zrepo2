...
class Config {
    ...
    constructor() {
        ...
        this.mqttPort = process.env.MQTT_PORT || null;
        ...
        try {
            ...
            if (mtime != this.lastConfigTime) {
                ...
                try {
                    ...
                    await (require("../lib/PluginAPI").init())
                    return this.config;
                } catch(error) {
                    console.error(error);
    ...
    buildConfig(c) {
        ...
        // mapa de preprocesadores de filas de dataSets
        this.dsPreprocessors = (c["process-groups"] || []).reduce((map, g) => {
            for (let process of (g.content || [])) {
                for (let trigger of (process.triggers || [])) {
                    if (trigger.type == "dataSet-preprocessor") {
                        let list = map[trigger.dataSet];
                        if (!list) {
                            list = [];
                            map[trigger.dataSet] = list;
                        }
                        list.push({trigger, process});                        
                    }
                }
            }
            return map;
    ...
    async getMQTTPubAuth() {
        await this.getChangedConfig();
        if (!this.config.mqtt || !this.config.mqtt.subUser) return null;
        return {user:this.config.mqtt.subUser, pwd:this.config.mqtt.subPwd}
    }
