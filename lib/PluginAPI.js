...
class PluginAPI {
    ...
    async init() {
        try {
            ...
            let col = await mongoWrite.collection("process_execution");   
            ...
            this.plugins = {};
            for (let pCode of Object.keys(plugins)) {
                this.plugins[pCode] = JSON.parse(JSON.stringify(plugins[pCode]));
            }
        } catch (error) {
            throw error;
        }
        ...
        try {
            ...
            app.post("/pluginAPI/finishProcess", async (req, res) => {
                try {
                    ...
                    await this.finishProcess(req.body.instanceId, req.body.description);
                    this.returnOK(res, {});
                } catch(error) {
                    this.returnError(res, error.toString());
                }
            ...
            app.get("/proxy/:plugin/*", async (req, res) => {
                ...
                let plugin = this.plugins[pluginCode];
                if (plugin) {
                    let originalUrl = req.originalUrl;
                    let p0 = originalUrl.indexOf("/", 7);
                    let newurl = plugin.callbackURL + originalUrl.substr(p0);
                    let proxyRequest = request(newurl);
                    proxyRequest.pipe(res);
                    proxyRequest.on("error", error => {
                        console.error("Error en proxy a " + pluginCode, error);
                    });
                } else {
                    this.returnError(res, "Plugin '" + pluginCode + "' no encontrado", 403);
                }
            });
            app.post("/proxy/:plugin/*.*", async (req, res) => {
                let pluginCode = req.params.plugin;
                let plugin = this.plugins[pluginCode];
                if (plugin) {
                    let originalUrl = req.originalUrl;
                    let p0 = originalUrl.indexOf("/", 7);
                    let newurl = plugin.callbackURL + originalUrl.substr(p0);
                    let proxyRequest = request.post(newurl);
                    proxyRequest.pipe(res);
                    proxyRequest.on("error", error => {
                        console.error("Error en proxy a " + pluginCode, error);
                    });
            ...
            col = await mongoWrite.collection("process_data");
        } catch(error) {
            throw error;
        }
    }
    ...
    async getProcessData(process) {
        try {
            let col = await mongoRead.collection("process_data");
    ...
    async setProcessData(process, data) {
        try {
            let col = await mongoWrite.collection("process_data");
    ...
    async getProcessMetadata(plugin, process) {
        try {
            let pluginDef = this.plugins[plugin];
            if (!pluginDef) throw "No se encontró la definición del Plugin '" + plugin + "'";
            let url = pluginDef.callbackURL + "/zcb/" + plugin + "/metadata/" + process;
            let md = await this.get(url);
            return md;
        } catch (error) {
            throw error;
        }
    ...
    async startProcess(email, plugin, process, params, name, trigger) {
        try {
            ...
            try {
                await this.post(pluginDef.callbackURL + "/zcb/" + plugin + "/exec", {}, {
    ...
    async getRunningInstances(instanceId) {
        try {
            let pis;
            let col = await mongoRead.collection("process_execution");            
            // instanceId puede ser un codigo de proceso (codigo-plugin.codigo-proceso), "_all_" o instanceId para busdcar "pares"
            if (instanceId.indexOf(".") > 0) {
                pis = await col.find({code:instanceId, running: true}, {projection:{_id:1, code:1, creator:1, params:1, started:1, name:1, logs:1}}).toArray();
                pis = pis.map(p => ({instanceId:p._id, code:p.code, creator:p.creator, params:p.params, started:p.started, name:p.name, logs:p.logs}));
            } else if (instanceId == "_all_") {
                pis = await col.find({running: true}, {projection:{_id:1, code:1, creator:1, params:1, started:1, name:1, logs:1}}).toArray();
                pis = pis.map(p => ({instanceId:p._id, code:p.code, creator:p.creator, params:p.params, started:p.started, name:p.name, logs:p.logs}));
            } else {
                ...
                pis = await col.find({code:pi.code, running: true}, {projection:{_id:1, code:1, creator:1, params:1, started:1, name:1, logs:1}}).toArray();
                pis = pis.filter(p => (p._id != instanceId)).map(p => ({instanceId:p._id, code:p.code, creator:p.creator, params:p.params, started:p.started, name:p.name, logs:p.logs}));
            }
            let now = Date.now();
            for (let pi of pis) {
                let maxTime = pi.started;
                for (let log of (pi.logs || [])) {
                    if (log.time > maxTime) maxTime = log.time;
                }
                pi.idleTime = now - maxTime;
                delete pi.logs;
            }
            return pis;
    } catch (error) {
    ...
    async runDataSetPreprocessor(process, rows, trigger) {
        try {
            let pluginDef = this.plugins[process.plugin];
            if (!pluginDef) throw "No se encontró la definición del Plugin '" + plugin + "'";
            ...
            try {
                let newRows = await this.post(pluginDef.callbackURL + "/zcb/" + process.plugin + "/execSync", {}, {
    ...
    async finishProcess(instanceId, description) {
        try {
            if (description) await this.addLog(instanceId, "E", description);
            let col = await mongoWrite.collection("process_execution");
    ...
    async findExecutions(processes, pluginCode, processCode, start, end, onlyRunning, withErrors, withErrorsOrWarnings) {
        try {
            ...
            let col = await mongoRead.collection("process_execution");
            let rows = await col.find(filter, {projection:{_id:1, started:1, finished:1, name:1, errors:1, warnings:1, running:1, canceling:1}}).sort({started:1}).toArray();
            return rows;
        } catch (error) {
            throw error;
        }
    ...
    async cronTrigger(process, trigger) {
        try {
            ...
            let col = await mongoWrite.collection("cron_procs_trigger");
            let r;
            try {
            ...
            let metadata = await this.getProcessMetadata(process.plugin, process.code);
            if (!metadata) throw "No se puede obtener la metadata del proceso '" + process.plugin + "." +  process.code + "'";
            let params = {};
            for (let param of (metadata.params || [])) {
            ...
            console.error("Error disparando proceso cron");
            console.error(error);
            // throw error;
        }
    }
