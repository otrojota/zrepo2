...
async function startHTTPServer() {
    try {
        let version = "?";
        try {
            let txt = (require("fs")).readFileSync("./build.sh").toString();
            txt = txt.split("\n")[0];
            let p = txt.indexOf("=");
        ...
        zServer.registerModule("fs", fileSystem);
        ...
        await (require("./lib/PluginAPI")).registerEndPoints(app);
        ...
        httpServer.listen(port, "::", _ => {
            console.log("[ZRepo HTTP Server " + version + "] Listenning at Port " + port);
            logs.info("[ZRepo HTTP Server " + version + "] Listenning at Port " + port)
        });
        if (config.mqttPort) {
